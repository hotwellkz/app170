...
export const deleteTransaction = async (transactionId: string): Promise<void> => {
  ...
  try {
    ...
    // Обновляем баланс исходной категории
    const sourceCategoryRef = doc(db, 'categories', categoryId);
    ...
    if (sourceCategorySnap.exists()) {
      ...
      // Отправляем уведомление в Telegram только для не-складских операций
      if (!isWarehouseOperation) {
        await sendTelegramNotification(
          formatTransactionMessage(
            transactionData.fromUser,
            transactionData.toUser,
            Math.abs(amount),
    ...
    // Обновляем баланс целевой категории, если есть связанная транзакция
